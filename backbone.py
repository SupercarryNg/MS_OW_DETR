...
if __name__ == '__main__':
    ...
    test_tensor = [torch.randn(3, 224, 224), torch.randn(3, 256, 224)]
    nested_list = nested_tensor_from_tensor_list(test_tensor)
    print(backbone(nested_list).keys())
    for item in backbone(nested_list).keys():
        print(backbone(nested_list).get(item).tensors.shape)
